...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    if (BRANCH_NAME.startsWith("master")) {
                        echo "***** PERFORMING STEPS ON MASTER BRANCH *****"
                        env['RUN_BUILD_MASTER'] = true
                        env['environment'] = "prd"
                        updateVersion(true)
                        env['RUN_DEPLOY'] = false
                        env['BRANCH_VERSION'] = "master"
                    }
                    ...
                    env['url_docker'] = "758526784474.dkr.ecr.us-east-1.amazonaws.com/base-playwright-py-qa:${newVersion}"
                }
            ...
            steps {
                script {
                    ...
                    docker.withRegistry("${env.ECR_URL}",'ecr:us-east-1:ecr-private-registry'){
                        def app = docker.build("base-playwright-py-qa")
                        app.push(env['newVersion'])
                        app.push(env['BRANCH_VERSION'])
                    }
                }
            }
        }
        ...
        stage ('delete image'){
            ...
            steps{
                script{
                    removeImage('base-playwright-py-qa',env['newVersion'])
                }
            }
        }
...
def notifyBuild(String buildStatus = 'STARTED') {
    ...
    if (buildStatus == 'STARTED') {
        ...
        field.put('value', 'base-playwright-py-qa');
        fields.add(field);
    ...
    slackSend(attachments: attachments.toString(), channel: "#qas-build-jenkins") 
}
